...
exports.extend = (app) =>
{
  ...
  app.pathTo = (...parts) => path.join.apply(
    ...
    app.options && app.options.rootPath
      ? [app.options.rootPath, ...parts]
      : parts
  );

  /**
   * @param {(Array.<(string|null)>)} moduleNames
   * @param {function} setUpFunction
   */
  app.onModuleReady = (moduleNames, setUpFunction) =>
  {
    const remainingModuleNames = [...moduleNames];
    ...
    [...remainingModuleNames].forEach(checkModule);
    ...
    function setUpIfReady()
    {
      ...
      {
        sub.cancel();
        setImmediate(setUpFunction, ...moduleNames.map(moduleName => app[moduleName]));
      }
    }
  };
};
